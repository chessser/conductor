...
A `Conductor Request` is triage input. On grooming (`conductor triage`) it's
converted into either a single `Conductor Task` or decomposed into a
`Conductor Ordered Task` plus sub-tasks. This conversion can be
...
and `mode/*` are present AND — for `Conductor Ordered Task` sub-tasks — every
