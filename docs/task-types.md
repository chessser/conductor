...
Jira issue types** (`Forge Task` vs. `Forge Ordered Task` — see
...
A `Forge Task` backed by a **template**: a named, versioned prompt plus a
...
A `Forge Ordered Task` epic with `Forge Task` sub-tasks linked by native
Jira issue links (`Blocks`/`Blocked by`). The dispatcher:

1. Resolves the DAG from those links on each `forge sync` — pure logic,
...
3. On a sub-task's MR merging, `forge mr-poll` re-evaluates the DAG to
   unblock the next layer — poll, flip status, check what's now unblocked.

`forge ready` previews the current dispatchable set; for an ordered task,
